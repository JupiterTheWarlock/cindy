...
export function buildLinuxUpdateScript(params: LinuxUpdateScriptParams): string {
  ...
  return [
    ...
    '            continue',
    '        fi',
    '        # 父 bash 死了。安装链是否还在,只看记录在案的安装子进程',
    '        # (pkexec)是否存活——不扫进程组,避免把心跳自己的 sleep',
    '        # 等辅助进程误判成安装链,也不受无关 apt/dpkg 干扰。',
    '        if [ -f "$INSTALL_PID_FILE" ]; then',
    '            IPID=$(cat "$INSTALL_PID_FILE" 2>/dev/null || true)',
