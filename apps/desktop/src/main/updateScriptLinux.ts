...
export function buildLinuxUpdateScript(params: LinuxUpdateScriptParams): string {
  ...
  return [
    ...
    `echo updating $$ > ${qLock}`,
    'LOCK_PARENT=$$',
    // detached spawn 让本脚本成为进程组组长,整个安装链
    // (pkexec → elevated bash → apt/dpkg)都在同一进程组里。
    'LOCK_PGID=$$',
    '(',
    '    while true; do',
    ...
    '            continue',
    '        fi',
    '        # 父 bash 死了。安装链是否还在,只看本进程组里除自己之外',
    '        # 还有没有活着的成员——不按全系统进程名猜,既不误杀也不',
    '        # 把无关的 apt/dpkg 当成本次安装。',
    '        OTHERS=$(pgrep -g "$LOCK_PGID" 2>/dev/null | grep -v -w -e "$BASHPID" || true)',
    '        if [ -n "$OTHERS" ]; then',
    ...
    // 先放锁再拉起:否则新进程一启动就卡在 bootstrap 等锁循环里,
    // kill -0 看到的是「活着但被自己人的锁挡住」的假活。等锁的旧实例
    // 醒来后会 exec 自己重新加载新二进制(bootstrap 侧),不存在旧代码
    // 抢赢单实例锁的问题。
    `rm -f ${qLock}`,
    `nohup ${qExe} >/dev/null 2>&1 &`,
    'LAUNCHED_PID=$!',
