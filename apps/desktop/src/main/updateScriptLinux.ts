...
export function buildLinuxUpdateScript(params: LinuxUpdateScriptParams): string {
  ...
  return [
    ...
    '            continue',
    '        fi',
    '        # 父 bash 死了。若安装链(pkexec / apt-get / dpkg)还在跑,',
    '        # 锁必须继续持有——清掉会让新实例在文件替换中途启动。',
    '        # 此时心跳以自己的 PID 作持有者,安装结束前谁也不能进。',
    '        INSTALL_RUNNING=0',
    '        for PROG in pkexec apt-get dpkg; do',
    '            if pgrep -x "$PROG" >/dev/null 2>&1; then',
    '                INSTALL_RUNNING=1',
    '                break',
    '            fi',
    '        done',
    '        if [ "$INSTALL_RUNNING" -eq 1 ]; then',
    `            echo updating $$ > ${qLock}`,
    `            sleep ${t.lockHeartbeatSeconds}`,
    '            continue',
    '        fi',
    '        # 父死且安装进程都不在:确实没有人在替换文件,清锁退出,',
    '        # 避免孤儿心跳把后续启动永久卡住。',
    `        rm -f ${qLock}`,
    '        exit 0',
    '    done',
