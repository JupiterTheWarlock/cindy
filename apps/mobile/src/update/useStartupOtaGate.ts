...
export function useStartupOtaGate(channel: UpdateChannel = 'release'): boolean {
  ...
  // 的请求发到上一个账号的 canary/stable 指针。release 也会显式写入共享
  // EAS-Client-ID 并清掉之前的 canary header，绝不退回原生生成的单安装标识。
  useEffect(() => {
    if (!baseEnabled || configuredChannelRef.current === channel) return;
    try {
      configureUpdateUrl();
    } catch {
  ...
  }, [configureUpdateUrl, baseEnabled, channel]);
  ...
  }, [baseEnabled, configureUpdateUrl]);
