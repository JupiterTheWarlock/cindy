...
- 模型配置浮层的 Harness 胶囊改为紧凑的两行选项，由 ModelHarnessPicker 共用子组件呈现。图标对齐名称首行，次行显示实际协议与“原生支持 / 兼容模式 / 协议待确认”；推荐项在名称旁标“推荐”，无需悬停阅读。选项上方显示模型原生协议，与设置页共用协议名称。
- 当前选择仅由实际 config.engine 决定，只给当前项勾与选中背景。推荐项排序靠前，但推荐本身不画第二个选中态、不触发任何配置写入。
