...
PR-6 另需 §10 圆角裁决关闭；PR-7 升阻断需 §8 管理员审核。

| # | 内容 | 风险类别 |
| --- | --- | --- |
| 1 | 治理合同 + 处置表（本 PR） | 零视觉（纯文档） |
| 2 | 生产 UI inventory + 主题兼容冻结测试 | 零视觉 |
| 3 | 最小语义 Token 层（影子层，零运行时） | 零视觉 |
| 4 | Button + Input / FormField 标准件（落入既有 `components/ui/`） | 有意可见 |
| 5 | AI 对话区 Pattern（Tool Call / Reasoning / Message / Attachment） | 有意可见 |
| 6 | Permission Pattern（裁决关闭后） | 有意可见 |
| 7 | 只拦新增的棘轮 | CI 门禁 |
| 8 | Terrazzo 生产生成切换 | 零视觉（大型基建） |
| 9 | Mobile 扩展 | 可见 + 基建 |
